...
def main(args):
    ...
    if args.data is not None:
        ...
        for i, ex in enumerate(data):
            query = ex.get("question") or ex.get("instruction")
            if query is None:
                raise ValueError(
                    f"Example at index {i} has neither 'question' nor 'instruction' field."
                )
            ex["ctxs"] = retriever.search_document(query, args.n_docs)

        if args.output_dir is None:
            raise ValueError("Please provide --output_dir when using --data.")
    ...
    if args.query is None:
        raise ValueError("Please provide either --query or --data.")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Single query string for retrieval",
    )
    parser.add_argument(
        "--data",
        type=str,
        default=None,
        help="Input .json/.jsonl file. Each instance should contain `question` or `instruction`.",
    )
