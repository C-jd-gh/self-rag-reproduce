...
def call_model_rerank_w_scores_batch(prompt, evidences, model, max_new_tokens=15,
    ...
    # If retrieval is selected but no evidence exists, fallback to no-retrieval.
    if do_retrieve is True and len(evidences) == 0:
        do_retrieve = False
...
def main():
    ...
    for i, row in tqdm(enumerate(input_data)):
        ...
        if isinstance(pred, str) and len(pred) > 0 and (pred[0] == "#" or pred[0] == ":"):
            pred = pred[1:]
